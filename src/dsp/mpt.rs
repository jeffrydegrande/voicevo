...
pub fn max_phonation_time_secs(contour: &[PitchFrame], hop_size_ms: f32) -> f32 {
    ...
    // Bridge gaps up to 500ms caused by pitch detector dropouts on breathy voices.
    //
    // MPT measures sustained phonation — the patient holds a vowel as long as
    // possible. Any gap < 500ms is either a detector artifact (breathy signal
    // loses periodicity momentarily) or a brief voice break that's still part
    // of the same phonation attempt. Only a gap > 500ms indicates the patient
    // actually stopped (e.g., took a breath), which should end the measurement.
    //
    // This threshold matches the upper bound of voice_breaks.rs (50-500ms = break,
    // > 500ms = intentional pause). For MPT we treat everything up to the pause
    // threshold as continuous phonation.
    let max_gap_frames = (500.0 / hop_size_ms) as usize;
